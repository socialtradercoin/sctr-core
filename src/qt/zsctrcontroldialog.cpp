...
#include "ui_zsctrcontroldialog.h"
...
std::set<std::string> ZSctrControlDialog::setSelectedMints;
std::set<CMintMeta> ZSctrControlDialog::setMints;

bool CZSctrControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZSctrControlDialog::COLUMN_DENOMINATION || column == ZSctrControlDialog::COLUMN_VERSION || column == ZSctrControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZSctrControlDialog::ZSctrControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSctrControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSctrControlDialog::~ZSctrControlDialog()
{
    delete ui;
}

void ZSctrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSctrControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZSctrControlWidgetItem* itemDenom(new CZSctrControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZSctrControlWidgetItem *itemMint = new CZSctrControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zsctrTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zsctrTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSCTR.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSCTR seed used to mint this zSCTR is not the same as currently hold in the wallet";
            else
...
void ZSctrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSctrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSctr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSctrControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSctrControlDialog::GetSelectedMints()
{
...
void ZSctrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
