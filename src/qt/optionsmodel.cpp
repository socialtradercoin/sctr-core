...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SCTR);
    ...
    if (!settings.contains("nAnonymizeSctrAmount"))
        settings.setValue("nAnonymizeSctrAmount", 1000);

    nAnonymizeSctrAmount = settings.value("nAnonymizeSctrAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSctrAmount"))
        SoftSetArg("-anonymizesctramount", settings.value("nAnonymizeSctrAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in sctr.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSctrAmount:
            return QVariant(nAnonymizeSctrAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeSctrAmount:
            nAnonymizeSctrAmount = value.toInt();
            settings.setValue("nAnonymizeSctrAmount", nAnonymizeSctrAmount);
            emit anonymizeSctrAmountChanged(nAnonymizeSctrAmount);
            break;
        case CoinControlFeatures:
