                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zSCTR ought to be enough for anybody." - Bill Gates, 2017
    ui->zSCTRpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSCTRSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSCTRpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSCTR_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSCTR is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zSCTR, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSCTR...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSCTR in ") +
...
void PrivacyDialog::on_pushButtonSpendzSCTR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSCTR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zSCTR, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zSCTR
        sendzSCTR();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSCTR
    sendzSCTR();
}

void PrivacyDialog::on_pushButtonZSctrControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZSctrControlDialog* zSctrControl = new ZSctrControlDialog(this);
    zSctrControl->setModel(walletModel);
    zSctrControl->exec();
}

void PrivacyDialog::setZSctrControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSctrSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSCTR()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Sctr Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSCTRpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSCTR is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SCTR </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSCTR</b>";
    ...
    // use mints from zSCTR selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSctrControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZSctrControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zSCTR was spent successfully update the addressbook with the label
    ...
    // Clear zsctr selector in case it was used
    ZSctrControlDialog::setSelectedMints.clear();
    ui->labelzSctrSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSCTR Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Sctr, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSCTR Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zsctrTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSCTR </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSCTR "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSCTR "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zSCTR "));
    ui->labelzSCTRAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zSCTR </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zSCTR </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zSCTR </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzSCTRSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzSCTR->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zSCTR
        ui->pushButtonMintzSCTR->setEnabled(false);
        ui->pushButtonMintzSCTR->setToolTip(tr("zSCTR is currently disabled due to maintenance."));

        // Spend zSCTR
        ui->pushButtonSpendzSCTR->setEnabled(false);
        ui->pushButtonSpendzSCTR->setToolTip(tr("zSCTR is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zSCTR
        ui->pushButtonMintzSCTR->setEnabled(true);
        ui->pushButtonMintzSCTR->setToolTip(tr("PrivacyDialog", "Enter an amount of SCTR to convert to zSCTR", 0));

        // Spend zSCTR
        ui->pushButtonSpendzSCTR->setEnabled(true);
        ui->pushButtonSpendzSCTR->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
