...
public:
    ...
    bool WriteZSCTRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSCTRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSCTRSeed_deprecated(uint256& seed);
    bool EraseZSCTRSeed();
    bool EraseZSCTRSeed_deprecated();

    bool WriteZSCTRCount(const uint32_t& nCount);
    bool ReadZSCTRCount(uint32_t& nCount);
