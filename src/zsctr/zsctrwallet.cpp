...
CzSCTRWallet::CzSCTRWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsctr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSCTRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSCTRSeed_deprecated()) {
                    LogPrintf("%s: Updated zSCTR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsctr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsctr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSCTRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSCTRCount(nCountLastUsed);
    else if (!walletdb.ReadZSCTRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSCTRWallet::Lock()
{
    seedMaster = 0;
}

void CzSCTRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSCTRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSCTR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSCTRWallet::LoadMintPoolFromDB()
{
...
void CzSCTRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSCTRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSCTRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsctrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSCTRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSCTR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsctrTracker which also adds to database
    pwalletMain->zsctrTracker->Add(dMint, true);
...
void CzSCTRWallet::SeedToZSCTR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSCTRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSCTRWallet::GenerateDeterministicZSCTR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSCTRWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSCTRWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSCTRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
