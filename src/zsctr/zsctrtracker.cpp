...
CzSCTRTracker::CzSCTRTracker(std::string strWalletFile)
{
...
CzSCTRTracker::~CzSCTRTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSCTRTracker::Init()
{
...
bool CzSCTRTracker::Archive(CMintMeta& meta)
{
...
bool CzSCTRTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSCTRTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSCTRTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSCTRTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzSCTRTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSCTRTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzSCTRTracker::GetSerialHashes()
{
...
CAmount CzSCTRTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSCTRTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSCTRTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSCTRTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSCTRTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSCTRTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSCTRTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSCTRTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSCTRTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSCTRTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSCTRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSCTRWallet* zSCTRWallet)
{
    bool iszSCTRWalletInitialized = (NULL != zSCTRWallet);
    CMintMeta meta;
    ...
    if (! iszSCTRWalletInitialized)
        zSCTRWallet = new CzSCTRWallet(strWalletFile);
    meta.isSeedCorrect = zSCTRWallet->CheckSeed(dMint);
    if (! iszSCTRWalletInitialized)
        delete zSCTRWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSCTRTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSCTRTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSCTRTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSCTRTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSCTRTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSCTRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSCTRWallet* zSCTRWallet = new CzSCTRWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zSCTRWallet);
        }
        delete zSCTRWallet;
        LogPrint("zero", "%s: added %d dzsctr from DB\n", __func__, listDeterministicDB.size());
    }
