...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSctrStake(spend));
    } else {
        ...
        CSctrStake* sctrInput = new CSctrStake();
        sctrInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(sctrInput);
    }
