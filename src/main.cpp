...
#include "zsctr/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "SCTR cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in usctr) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zSCTR spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zSCTR is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zSCTR spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zSCTR without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zSCTR spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSCTR spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zsctr");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zSCTR mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
         ...
         nSubsidy = 0 * COIN;
    } else if (nHeight <= 14600 && nHeight > 200) {
         nSubsidy = 0.25 * COIN;
    } else if (nHeight <= 24680 && nHeight > 14600) {
         nSubsidy = 0.5 * COIN;
    } else if (nHeight <= 34760 && nHeight > 24680) {
         nSubsidy = 0.75 * COIN;
    } else if (nHeight <= 54920 && nHeight > 34760) {
         nSubsidy = 1.25 * COIN;
    } else if (nHeight <= 75080 && nHeight > 54920) {
         nSubsidy = 1.75 * COIN;
    } else if (nHeight <= 95240 && nHeight > 75080) {
         nSubsidy = 2.25 * COIN;
    } else if (nHeight <= 115400 && nHeight > 95240) {
         nSubsidy = 2.75 * COIN;
    } else if (nHeight <= 145640 && nHeight > 115400) {
         nSubsidy = 3.25 * COIN;
    } else if (nHeight <= 175880 && nHeight > 145640) {
         nSubsidy = 3.75 * COIN;
    } else if (nHeight <= 206120 && nHeight > 175880) {
         nSubsidy = 4.25 * COIN;
    } else if (nHeight <= 236360 && nHeight > 206120) {
         nSubsidy = 4.75 * COIN;
    } else if (nHeight <= 279560 && nHeight > 236360) {
         nSubsidy = 5.25 * COIN;
    } else if (nHeight <= 322760 && nHeight > 279560) {
         nSubsidy = 5.75 * COIN;
    } else if (nHeight <= 365960 && nHeight > 322760) {
         nSubsidy = 6.25 * COIN;
    } else if (nHeight <= 409160 && nHeight > 365960) {
         nSubsidy = 6.75 * COIN;
    } else if (nHeight <= 538760 && nHeight > 409160) {
         nSubsidy = 7.25 * COIN;
    } else if (nHeight <= 668360 && nHeight > 538760) {
         nSubsidy = 7.75 * COIN;
    } else if (nHeight <= 797960 && nHeight > 668360) {
         nSubsidy = 8.25 * COIN;
    } else if (nHeight <= 927560 && nHeight > 797960) {
         nSubsidy = 8.75 * COIN;
    } else if (nHeight <= 1186760 && nHeight > 927560) {
         nSubsidy = 0.25 * COIN;
    } else if (nHeight <= 1445960 && nHeight > 1186760) {
         nSubsidy = 7 * COIN;
    } else if (nHeight <= 1705160 && nHeight > 1445960) {
         nSubsidy = 6 * COIN;
    } else if (nHeight > 1705160) {
         nSubsidy = 5 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZSCTRStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SCTR
...
void RecalculateZSCTRMinted()
{
    ...
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZSCTR..."), percent);
        }
...
void RecalculateZSCTRSpent()
{
    ...
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = (int)( (double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()) );
            uiInterface.ShowProgress(_("Recalculating spent ZSCTR..."), percent);
        }

        //Rewrite zSCTR supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
        ...
        //Remove spends from zSCTR supply
...
bool RecalculateSCTRSupply(int nHeightStart)
{
    ...
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating SCTR supply..."), percent);
        }
...
bool UpdateZSCTRSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zSCTR supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateSCTRSupply(Params().Zerocoin_StartHeight());
    }

    //Track zSCTR money supply in the block index
    if (!UpdateZSCTRSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zSCTR supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSctrSpent: %s\n",
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert SCTR to zSCTR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSCTR spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSCTR serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zSCTR stake block is less than allowed start height", __func__);

    if (CZSctrStake* zSCTR = dynamic_cast<CZSctrStake*>(stake)) {
        CBlockIndex* pindexFrom = zSCTR->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zSCTR stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zSCTR stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zSCTR->GetValue()));
        if (nChecksum200 != zSCTR->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zSCTR->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZSCTR() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zSCTR fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> sctrInputs;
        std::vector<CTxIn> zSCTRInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zSCTRInputs.push_back(stakeIn);
            }else{
                sctrInputs.push_back(stakeIn);
            }
        }
        const bool hasSCTRInputs = !sctrInputs.empty();
        const bool hasZSCTRInputs = !zSCTRInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasSCTRInputs)
                    ...
                    for (const CTxIn& sctrIn : sctrInputs){
                        if(sctrIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            do {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : sctrInputs) {
            ...
            // Now that this loop if completed. Check if we have zSCTR inputs.
            if(hasZSCTRInputs){
                for (const CTxIn& zSctrInput : zSCTRInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zSctrInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zsctr");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSCTR mints and %d zSCTR spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SCTR: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zsctr", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zsctr", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
